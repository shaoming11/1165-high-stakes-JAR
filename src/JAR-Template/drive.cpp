...
void Drive::turn_to_angle(float angle, float turn_max_voltage, float turn_settle_error, float turn_settle_time, float turn_timeout, float turn_kp, float turn_ki, float turn_kd, float turn_starti){
  PID turnPID(reduce_negative_180_to_180(angle - get_absolute_heading()), turn_kp, turn_ki, turn_kd, turn_starti, turn_settle_error, turn_settle_time-100, turn_timeout);
...
void Drive::drive_side_distance(float distance, float drive_max_voltage, float left_kp, float left_ki, float left_kd, float left_starti, float right_kp, float right_ki, float right_kd, float right_starti) {
  drive_side_distance(distance, get_absolute_heading(), drive_max_voltage, heading_max_voltage, drive_settle_error, drive_settle_time, drive_timeout, left_kp, left_ki, left_kd, left_starti, right_kp, right_ki, right_kd, right_starti, heading_kp, heading_ki, heading_kd, heading_starti);
}

void Drive::drive_side_distance(float distance, float heading, float drive_max_voltage, float heading_max_voltage, float drive_settle_error, float drive_settle_time, float drive_timeout, float left_kp, float left_ki, float left_kd, float left_starti, float right_kp, float right_ki, float right_kd, float right_starti, float heading_kp, float heading_ki, float heading_kd, float heading_starti) {
  // Initialize separate PIDs for left and right sides
  PID leftDrivePID(distance, left_kp, left_ki, left_kd, left_starti, drive_settle_error, drive_settle_time, drive_timeout);
  PID rightDrivePID(distance, right_kp, right_ki, right_kd, right_starti, drive_settle_error, drive_settle_time, drive_timeout);
  PID headingPID(reduce_negative_180_to_180(heading - get_absolute_heading()), heading_kp, heading_ki, heading_kd, heading_starti);

  // Starting positions
  float start_left_position = get_left_position_in();
  float start_right_position = get_right_position_in();
  ...
  while (!leftDrivePID.is_settled() || !rightDrivePID.is_settled()) {
    ...
    // Calculate errors
    float left_error = distance + start_left_position - left_position;
    float right_error = distance + start_right_position - right_position;
    float heading_error = reduce_negative_180_to_180(heading - get_absolute_heading());

    // Compute PID outputs
    float left_output = leftDrivePID.compute(left_error);
    float right_output = rightDrivePID.compute(right_error);
    float heading_output = headingPID.compute(heading_error);

    // Clamp the outputs
    left_output = clamp(left_output + heading_output, -drive_max_voltage, drive_max_voltage);
    right_output = clamp(right_output - heading_output, -drive_max_voltage, drive_max_voltage);
